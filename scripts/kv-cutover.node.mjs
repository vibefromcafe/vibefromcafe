...
test("reconciliation detects metadata, expiration, and binary encoding drift", () => {
  const records = [
    { key: "unknown:binary", value: "AAEC", base64: true, expiration: 1_800_000_000, metadata: { version: 1 } },
  ];

  for (const changedRecord of [
    { ...records[0], metadata: { version: 2 } },
    { ...records[0], expiration: 1_900_000_000 },
    { ...records[0], base64: false },
  ]) {
    assert.equal(
      buildReconciliation(snapshot("source", records), snapshot("staging", [changedRecord])).matches,
      false,
    );
  }
});
...
test("live preflight refuses a non-staging alias", () => {
  assert.throws(
    () => validateLiveDestinationPreflight({
      destinationBefore: snapshot("staging", [], "staging-id"),
      liveDestination: snapshot("production", [], "staging-id"),
      destinationId: "staging-id",
    }),
