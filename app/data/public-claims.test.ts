...
/**
 * Word-boundary checks for deployment badges. Avoid matching explanatory
 * negatives like "bukan … di-deploy" by requiring the standalone badge form.
 */
const forbiddenBadgePatterns: RegExp[] = [/\bDeployed\b/, /\bVCFC\s*\/\s*00\d\b/i];
...
function collectModuleText(): string {
  ...
  for (const chapter of publicChapters) {
    parts.push(chapter.name, chapter.scope, chapter.focus, chapter.detail, chapter.to ?? "");
  }
...
describe("public claims data", () => {
  ...
  it("omits member counts and universal active status from chapters", () => {
    ...
    for (const chapter of publicChapters) {
      ...
      expect(chapter.detail.toLowerCase()).not.toBe("active group");
      if (chapter.status === "open_chapter") {
        expect(chapter.to).toBeTruthy();
      }
    }
  });
...
describe("claim-owned route surfaces", () => {
  it("do not reintroduce unsupported quantified, deployment, or faux-link claims", () => {
